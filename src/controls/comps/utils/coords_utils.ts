 ...
 */
function getPointXorYOnBezier(t:number, start:number, startCtrl:number, endCtrl:number,end: number):number {
    const t2 = t * t;
    const t3 = t2 * t;
    return start + (-start * 3 + t * (3 * start - start * t)) * t
...
function getBezierLength(c1: Coord, c2: Coord, segmentsCount = 50):number {
	...
	for (let i = 1; i < pointsAlongCurve.length; i++) {
		lengthSum += getDistance(pointsAlongCurve[i-1].x, pointsAlongCurve[i-1].y, pointsAlongCurve[i].x, pointsAlongCurve[i].y);
	}
